...
GO
CREATE PROC insertEmployee (@ssn int,  @f_name varchar(50), @l_name varchar(50), @birth_date DATE,
...
CREATE PROC updateEmployee (@ssn int,  @f_name varchar(50), @l_name varchar(50), @birth_date DATE,
		@address VARCHAR(50),	@postal_code VARCHAR(50), @city VARCHAR(50),
		@job VARCHAR(50),	@phone_number INT,	@mail VARCHAR(100))
AS
BEGIN
...
CREATE PROC p_criaInter (@description VARCHAR(50), @price DECIMAL(10,2), @startDate DATE, @endDate DATE, @frequency INT, @asset_id INT, @skillDescription VARCHAR(50))
AS
BEGIN
	...
	EXEC @freeTeam = dbo.get_code_from_team @skillDescription 
...
BEGIN
    ...
    SELECT intervention_code, description 
    FROM INTERVENTION 
...
GO
CREATE PROC updateStateIntervention (@intervention_code INT,@state VARCHAR(50), @endDate DATE)
AS
BEGIN
  SET TRANSACTION ISOLATION LEVEL READ COMMITTED
  BEGIN TRANSACTION
