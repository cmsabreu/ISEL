    ...
    class Program
    {
        static void Main(string[] args)
        {
            ...
            using (Context ctx = new Context(connectionString))
            {
                ...
                foreach (var country in ctx.Countries.FindAll())
                {
           ...
           using (Context ctx = new Context(connectionString))
            {
                ...
                Console.WriteLine("FindAll");
                foreach (var course in ctx.Courses.FindAll())
                {
                    Console.WriteLine("Course: {0}-{1}", course.Id, course.Name);
                }
                Console.WriteLine("Find");
                foreach (var course in ctx.Courses.Find(ct => ct.Name.Contains("Information")))
                {
                    Console.WriteLine("Course: {0}-{1}", course.Id, course.Name);
                }
            ...
            using (Context ctx = new Context(connectionString))
            {
                ...
                Console.WriteLine("FindAll");
                foreach (var student in ctx.Students.FindAll())
                {
                    Console.WriteLine("Student: {0}-{1}-{2}-{3}-{4}", student.Number, student.Name, student.Sex, student.DateOfBirth, student.HomeCountry.Name);
                }
                Console.WriteLine("Find");
                foreach (var student in ctx.Students.Find(ct => ct.Name.Contains("Two")))
                {
                    Console.WriteLine("Student: {0}-{1}-{2}-{3}-{4}", student.Number, student.Name, student.Sex, student.DateOfBirth, student.HomeCountry.Name);
                }
